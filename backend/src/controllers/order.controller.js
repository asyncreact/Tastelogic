...
import { getActiveReservationByUserId } from "../repositories/reservation.repository.js"; // ⭐ NUEVO
...
export const showOrder = async (req, res, next) => {
  try {
    ...
    if (!order) {
      const error = new Error("No encontramos tu pedido. Por favor, verifica tus datos.");
      error.status = 404;
      throw error;
    }
...
export const addOrder = async (req, res, next) => {
  try {
    const {
      reservation_id,
      table_id, // ⭐ Opcional para dine-in
      order_type,
      status,
      payment_method,
      payment_status,
      special_instructions,
      items, // ⭐ Solo necesita [{menu_item_id, quantity, special_notes}]
    } = req.body;

    // Customer crea pedidos para sí mismo, admin DEBE especificar user_id
    let user_id;
    if (req.user.role === "customer") {
      user_id = req.user.id;
    } else {
      if (!req.body.user_id) {
        const error = new Error("Como administrador, debes especificar el ID del usuario para crear el pedido");
        error.status = 400;
        throw error;
      }
    ...
    if (!order_type) {
      const error = new Error("Por favor, especifica el tipo de pedido (dine-in, takeout, delivery)");
      error.status = 400;
      throw error;
    }
    ...
    if (!validOrderTypes.includes(order_type)) {
      const error = new Error("Tipo de pedido inválido. Elige entre: comer aquí (dine-in), para llevar (takeout) o entrega (delivery)");
      error.status = 400;
      throw error;
    }
    ...
    // ⭐ LÓGICA: Asignar mesa automáticamente desde reserva si es dine-in
    ...
    if (order_type === "dine-in") {
      if (!table_id) {
        ...
        const activeReservation = await getActiveReservationByUserId(Number(user_id));
        
        if (activeReservation) {
          // ⭐ Asignar automáticamente la mesa de la reserva
          finalTableId = activeReservation.table_id;
          finalReservationId = activeReservation.id;
          autoAssignedTable = true;
        } else {
          // Si no hay reserva activa y no se proporcionó table_id
          const error = new Error("Para pedidos dine-in debes tener una reserva activa o especificar el número de mesa");
          error.status = 400;
          throw error;
        }
      } else {
        // Si se proporcionó table_id, validar que existe y está activa
    ...
    // ⭐ LÓGICA: Validar items y obtener precio automáticamente del menú
    let calculatedTotal = 0;
    const validatedItems = [];

    for (const item of items) {
      if (!item.menu_item_id || !item.quantity) {
        const error = new Error("Cada item debe tener menu_item_id y quantity");
        error.status = 400;
        throw error;
      }
      ...
      if (!menuItem) {
        const error = new Error(`El item del menú con ID ${item.menu_item_id} no existe`);
        error.status = 404;
        throw error;
      }

      if (!menuItem.is_available) {
        const error = new Error(`Lo sentimos, "${menuItem.name}" no está disponible en este momento`);
        error.status = 400;
        throw error;
      }

      // ⭐ OBTENER EL PRECIO DESDE EL MENÚ (no del request)
      ...
      validatedItems.push({
        ...
        quantity: quantity,
        unit_price: unit_price, // ⭐ Precio obtenido desde el menú
    ...
    // ⭐ Mensaje mejorado si se asignó mesa automáticamente
    let successMessage = `¡Pedido creado! Tipo: ${orderTypeNames[order_type]}, Total: RD$${calculatedTotal.toFixed(2)}`;
    if (autoAssignedTable) {
      successMessage += ` - Mesa asignada automáticamente desde tu reserva`;
    }
...
export const editOrder = async (req, res, next) => {
  try {
    ...
    if (isNaN(order_id) || order_id <= 0) {
      const error = new Error("No se pudo encontrar el pedido que deseas modificar");
      error.status = 400;
      throw error;
    }
    ...
    if (req.user.role === "customer" && existing.user_id !== req.user.id) {
      const error = new Error("No tienes permiso para modificar este pedido");
      error.status = 403;
      throw error;
    }

    // ⭐ VALIDACIÓN: No permitir editar pedidos cancelados
    if (existing.status === 'cancelled') {
      const error = new Error("No puedes modificar un pedido que ya fue cancelado");
      error.status = 400;
      throw error;
    }

    if (req.body.user_id && req.user.role !== "admin") {
      const error = new Error("No tienes permiso para cambiar el usuario del pedido");
      error.status = 403;
      throw error;
    }
    ...
    const update_data = {
      ...
      ...(req.body.special_instructions !== undefined && { special_instructions: req.body.special_instructions }),
    };

    if (req.user.role === "admin") {
      if (req.body.status !== undefined) update_data.status = req.body.status;
      if (req.body.payment_status !== undefined) update_data.payment_status = req.body.payment_status;
...
export const updateStatus = async (req, res, next) => {
  try {
    ...
    if (!status) {
      const error = new Error("Por favor, selecciona un estado para el pedido");
      error.status = 400;
      throw error;
    }
    ...
    // ⭐ VALIDACIÓN: Prevenir cambios en pedidos cancelados
    if (existing.status === 'cancelled' && status !== 'cancelled') {
      const error = new Error("No puedes cambiar el estado de un pedido cancelado");
      error.status = 400;
      throw error;
    }

    if (existing.status === status) {
      const error = new Error(`Tu pedido ya está ${statusNames[status]}. No se realizaron cambios.`);
      error.status = 400;
      throw error;
    }
...
export const updatePayment = async (req, res, next) => {
  try {
    ...
    if (!validPaymentStatuses.includes(payment_status)) {
      const error = new Error(
        `El estado de pago seleccionado no es válido. Elige entre: ${Object.values(paymentStatusNames).join(", ")}`
      );
      error.status = 400;
      throw error;
    }
    ...
    if (existing.payment_status === payment_status) {
      const error = new Error(`El pago ya está ${paymentStatusNames[payment_status]}. No se realizaron cambios.`);
      error.status = 400;
      throw error;
    }
...
export const cancelOrderHandler = async (req, res, next) => {
  try {
    ...
    if (isNaN(order_id) || order_id <= 0) {
      const error = new Error("No se pudo encontrar el pedido que deseas cancelar");
      error.status = 400;
      throw error;
    }
    ...
    if (existing.status === "cancelled") {
      const error = new Error("Este pedido ya fue cancelado anteriormente");
      error.status = 400;
      throw error;
    }

    if (existing.status === "completed") {
      const error = new Error("No puedes cancelar un pedido que ya fue completado");
      error.status = 400;
      throw error;
    }
...
export const removeOrder = async (req, res, next) => {
  try {
    ...
    if (isNaN(order_id) || order_id <= 0) {
      const error = new Error("No se pudo encontrar el pedido que deseas eliminar");
      error.status = 400;
      throw error;
    }
...
export const orderStatsByType = async (req, res, next) => {
  try {
    ...
    if (!stats || stats.length === 0) {
      const error = new Error("No hay datos disponibles para mostrar estadísticas por tipo");
      error.status = 404;
      throw error;
    }
    ...
    return successResponse(
      res,
      `Estadísticas generadas: ${stats.length} tipo${stats.length > 1 ? 's' : ''}`,
      { statistics: stats, count: stats.length }
    );
  } catch (err) {
    next(err);
...
export const topSellingItems = async (req, res, next) => {
  try {
    ...
    if (!items || items.length === 0) {
      const error = new Error("No hay datos disponibles para mostrar items más vendidos");
      error.status = 404;
      throw error;
    }
