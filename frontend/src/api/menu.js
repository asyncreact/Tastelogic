...
/**
 * 🆕 Obtiene el tiempo de preparación de un plato (ruta pública)
 * @param {string} id - ID del plato
 */
export const getPublicItemPrepTime = (id) =>
  axios.get(`${API_URL}/menu/public/items/${id}/prep-time`);
...
// ============================================================
// ⏱️ TIEMPOS DE PREPARACIÓN (requieren autenticación)
// ============================================================

/**
 * Obtiene el tiempo de preparación de un plato específico (autenticado)
 * @param {string} id - ID del plato
 * @param {string} token - Token de autenticación
 */
export const getItemPrepTime = (id, token) =>
  axios.get(`${API_URL}/menu/items/${id}/prep-time/auth`, {
    headers: { Authorization: `Bearer ${token}` }
  });

/**
 * Obtiene los tiempos de preparación de todos los platos (solo admin)
 * @param {string} token - Token de autenticación
 */
export const getAllItemsPrepTimes = (token) =>
  axios.get(`${API_URL}/menu/prep-times/all`, {
    headers: { Authorization: `Bearer ${token}` }
  });

/**
 * Actualiza el tiempo de preparación de un plato (solo admin)
 * @param {string} id - ID del plato
 * @param {Object} data - Datos del tiempo de preparación
 * @param {string} token - Token de autenticación
 */
export const updateItemPrepTime = (id, data, token) =>
  axios.patch(`${API_URL}/menu/items/${id}/prep-time`, data, {
    headers: { Authorization: `Bearer ${token}` }
  });
